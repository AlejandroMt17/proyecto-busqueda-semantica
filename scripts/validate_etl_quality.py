...
Comprueba: conteos, nulos en columnas críticas, chunk_id >= 0, text_len acorde al texto.

Ejemplo (local, lee desde MinIO):

  spark-submit --packages org.apache.hadoop:hadoop-aws:3.3.4,com.amazonaws:aws-java-sdk-bundle:1.12.262 ^
    scripts/validate_etl_quality.py --input-glob "s3a://semantic-raw/features/run_date=2026-05-12/**/*.csv" ^
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    p.add_argument(
        "--input-glob",
        required=True,
        help='CSV gzip del ETL, ej. s3a://semantic-raw/features/run_date=2026-05-12/*.csv',
    )
    ...
    p.add_argument(
        ...
        type=int,
        default=1,
        help="Mínimo de filas esperadas (falla si hay menos).",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        required = {"doc_id", "chunk_id", "title", "text_chunk", "run_date", "text_len"}
        ...
        bad_len = df.filter(
            F.col("text_len").isNull()
            | (F.col("text_len") < 0)
            | (F.col("text_len") > F.length(F.coalesce(F.col("text_chunk"), F.lit(""))))
        ).count()
        if bad_len:
            logger.error("text_len inconsistente en %s filas", bad_len)
            return 1
